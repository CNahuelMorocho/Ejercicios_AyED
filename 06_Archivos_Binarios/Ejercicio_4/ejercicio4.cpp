/* Dados dos archivos de enteros, previamente ordenados ascendentemente, confeccione un programa que guarde en un tercer archivo el contenido de ambos ordenado ascendentemente. */
...
void ordenarArchivo(FILE *arch, char path[], int n){
   int box[n], aux = 0;
   arch = fopen(path, "rb");
   fread(box, sizeof(int), n, arch);
   fclose(arch);
...
int main(){
   ...
   ordenarArchivo(archivo1, "archivo1.data", 3);
   mostrarArchivo(archivo1, "archivo1.data", 3);

   cout<<"Archivo 2:"<<endl;
   crearArchivo(archivo2, "archivo2.data", 3);
   ordenarArchivo(archivo1, "archivo2.data", 3);
   mostrarArchivo(archivo2, "archivo2.data", 3);

   cout<<"Archivo 3:"<<endl;
   //En este programa no tendria que intercalar los archivos, simplemente guardarlos, pero como ya esta el algoritmo implementado lo dejo asi. Despues se va cambiar en "ordenarArchivo".
   intercalarArchivos(archivo1, archivo2, archivo3, "archivo1.data", "archivo2.data", "archivo3.data", 3);
   ordenarArchivo(archivo1, "archivo3.data", 6);
